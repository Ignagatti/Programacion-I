...
# --- Asegurar las 9 mesas iniciales en el Excel ---
ids_existentes = [fila[0].value for fila in wreservas.iter_rows(min_row=2) if fila[0].value is not None]
...
class Restaurante:
    ...
    def agregar_mesa(self):
        ...
        def guardar_capacidad():
            ...
            ids_existentes = [fila[0].value for fila in hoja.iter_rows(min_row=2) if fila[0].value is not None]
            ...
            btn = Button(
                ...
                height=3,
                bg="green",
                command=lambda n=nuevo_id: Reservar_mesa(self.ventana, self.tipo, self, n)
            )
            btn.pack(side="left", padx=40, pady=10)
            self.estado_mesas[nuevo_id] = {"estado": "Libre", "nombre": "", "capacidad": capacidad}
